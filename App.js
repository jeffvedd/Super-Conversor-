...
const App = () => {
  ...
  const convert = () => {
    ...
    if (activeCategory === 'weight') {
      // converter qualquer unidade para kg
      const weightInKg = {
        'kg': value,
        'g': value / 1000,
        'lb': value * 0.453592,
    ...
    // ---------------- COMPRIMENTO ----------------
      ...
      convertedValue = {
        ...
        'ft': lengthInMeters / 0.3048,
        'in': lengthInMeters / 0.0254
      }[toUnit];
    }
  ...
  const swapUnits = () => {
    ...
    if (!isNaN(parseFloat(prevResult))) {
      setInputValue(prevResult);
      setResult(prevInput);
    }
  };
